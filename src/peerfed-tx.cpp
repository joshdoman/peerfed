...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " peerfed-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  peerfed-tx [options] <hex-tx> [commands]  Update hex-encoded bitcoin transaction\n"
                "or:     peerfed-tx [options] -create [commands]   Create hex-encoded bitcoin transaction\n"
                "\n";
...
static void MutateTxSign(CMutableTransaction& tx, const std::string& flagStr)
{
    ...
    UniValue prevtxsObj = registers["prevtxs"];
    {
        for (unsigned int previdx = 0; previdx < prevtxsObj.size(); previdx++) {
...

            {
                ...
                newcoin.out.amountType = UNKNOWN;
    ...
    for (unsigned int i = 0; i < mergedTx.vin.size(); i++) {
        ...
        const CAmountType& amountType = coin.out.amountType;

        SignatureData sigdata = DataFromTransaction(mergedTx, i, coin.out);
        // Only sign SIGHASH_SINGLE if there's a corresponding output:
        if (!fHashSingle || (i < mergedTx.vout.size()))
            ProduceSignature(keystore, MutableTransactionSignatureCreator(mergedTx, i, amountType, amount, nHashType), prevPubKey, sigdata);
