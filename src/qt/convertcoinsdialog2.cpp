...
#include <qt/forms/ui_convertcoinsdialog2.h>
...
ConvertCoinsDialog2::ConvertCoinsDialog2(const PlatformStyle *_platformStyle, QWidget *parent) :
    QDialog(parent, GUIUtil::dialog_flags),
    ui(new Ui::ConvertCoinsDialog2),
    clientModel(nullptr),
    model(nullptr),
    ...
    platformStyle(_platformStyle)
{
    ...
    connect(ui->clearButton, &QPushButton::clicked, this, &ConvertCoinsDialog2::clear);

    // Coin Control
    connect(ui->pushButtonCoinControl, &QPushButton::clicked, this, &ConvertCoinsDialog2::coinControlButtonClicked);
    connect(ui->checkBoxCoinControlChange, &QCheckBox::stateChanged, this, &ConvertCoinsDialog2::coinControlChangeChecked);
    connect(ui->lineEditCoinControlChange, &QValidatedLineEdit::textEdited, this, &ConvertCoinsDialog2::coinControlChangeEdited);
    ...
    connect(clipboardQuantityAction, &QAction::triggered, this, &ConvertCoinsDialog2::coinControlClipboardQuantity);
    connect(clipboardAmountAction, &QAction::triggered, this, &ConvertCoinsDialog2::coinControlClipboardAmount);
    connect(clipboardFeeAction, &QAction::triggered, this, &ConvertCoinsDialog2::coinControlClipboardFee);
    connect(clipboardAfterFeeAction, &QAction::triggered, this, &ConvertCoinsDialog2::coinControlClipboardAfterFee);
    connect(clipboardBytesAction, &QAction::triggered, this, &ConvertCoinsDialog2::coinControlClipboardBytes);
    connect(clipboardLowOutputAction, &QAction::triggered, this, &ConvertCoinsDialog2::coinControlClipboardLowOutput);
    connect(clipboardChangeAction, &QAction::triggered, this, &ConvertCoinsDialog2::coinControlClipboardChange);
    ...
    if (settings.value("fSendAmountType") == CASH)
        ui->customFee->setValue(settings.value("nConvertTransactionFeeCash").toLongLong());
    else
        ui->customFee->setValue(settings.value("nConvertTransactionFeeBond").toLongLong());
    minimizeFeeSection(settings.value("fConvertFeeSectionMinimized").toBool());

    GUIUtil::ExceptionSafeConnect(ui->convertButton, &QPushButton::clicked, this, &ConvertCoinsDialog2::convertButtonClicked);
}

void ConvertCoinsDialog2::setClientModel(ClientModel *_clientModel)
{
    this->clientModel = _clientModel;

    if (_clientModel) {
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &ConvertCoinsDialog2::updateNumberOfBlocks);
    }
}

void ConvertCoinsDialog2::setModel(WalletModel *_model)
{
    this->model = _model;

    if(_model && _model->getOptionsModel())
    {
        connect(_model, &WalletModel::balanceChanged, this, &ConvertCoinsDialog2::setBalance);
        connect(_model->getOptionsModel(), &OptionsModel::displayUnitChanged, this, &ConvertCoinsDialog2::refreshBalance);
        refreshBalance();

        // Coin Control
        connect(_model->getOptionsModel(), &OptionsModel::displayUnitChanged, this, &ConvertCoinsDialog2::coinControlUpdateLabels);
        connect(_model->getOptionsModel(), &OptionsModel::coinControlFeaturesChanged, this, &ConvertCoinsDialog2::coinControlFeatureChanged);
        ...
        connect(ui->confTargetSelector, qOverload<int>(&QComboBox::currentIndexChanged), this, &ConvertCoinsDialog2::updateSmartFeeLabel);
        connect(ui->confTargetSelector, qOverload<int>(&QComboBox::currentIndexChanged), this, &ConvertCoinsDialog2::coinControlUpdateLabels);

#if (QT_VERSION >= QT_VERSION_CHECK(5, 15, 0))
        connect(ui->groupFee, &QButtonGroup::idClicked, this, &ConvertCoinsDialog2::updateFeeSectionControls);
        connect(ui->groupFee, &QButtonGroup::idClicked, this, &ConvertCoinsDialog2::coinControlUpdateLabels);
#else
        connect(ui->groupFee, qOverload<int>(&QButtonGroup::buttonClicked), this, &ConvertCoinsDialog2::updateFeeSectionControls);
        connect(ui->groupFee, qOverload<int>(&QButtonGroup::buttonClicked), this, &ConvertCoinsDialog2::coinControlUpdateLabels);
#endif

        connect(ui->customFee, &BitcoinAmountField::valueChanged, this, &ConvertCoinsDialog2::coinControlUpdateLabels);
        connect(ui->optInRBF, &QCheckBox::stateChanged, this, &ConvertCoinsDialog2::updateSmartFeeLabel);
        connect(ui->optInRBF, &QCheckBox::stateChanged, this, &ConvertCoinsDialog2::coinControlUpdateLabels);
...
ConvertCoinsDialog2::~ConvertCoinsDialog2()
{
    QSettings settings;
...
bool ConvertCoinsDialog2::PrepareSendText(QString& question_string, QString& informative_text, QString& detailed_text)
{
    ...
    CAmountType amount_type = getFeeType();
    ...
    question_string.append(QString("<b>%1</b>: <b>%2</b>").arg(tr("Total Amount"))
        .arg(BitcoinUnits::formatHtmlWithUnit(unit, totalAmount)));
...
void ConvertCoinsDialog2::presentPSBT(PartiallySignedTransaction& psbtx)
{
    // Serialize the PSBT
...
bool ConvertCoinsDialog2::signWithExternalSigner(PartiallySignedTransaction& psbtx, CMutableTransaction& mtx, bool& complete) {
...
void ConvertCoinsDialog2::convertButtonClicked([[maybe_unused]] bool checked)
{
...
void ConvertCoinsDialog2::clear()
{
    m_current_transaction.reset();
...
void ConvertCoinsDialog2::reject()
{
    clear();
}

void ConvertCoinsDialog2::accept()
{
    clear();
}

void ConvertCoinsDialog2::updateTabsAndLabels()
{
    setupTabChain(nullptr);
    coinControlUpdateLabels();
}

QWidget *ConvertCoinsDialog2::setupTabChain(QWidget *prev)
{
...
void ConvertCoinsDialog2::setBalance(const interfaces::WalletBalances& balances)
{
    if(model && model->getOptionsModel())
    {
...
void ConvertCoinsDialog2::refreshBalance()
{
...
void ConvertCoinsDialog2::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    QPair<QString, CClientUIInterface::MessageBoxFlags> msgParams;
    // Default to a warning message, override if error message is needed
    msgParams.second = CClientUIInterface::MSG_WARNING;

    // This comment is specific to ConvertCoinsDialog2 usage of WalletModel::SendCoinsReturn.
...
void ConvertCoinsDialog2::minimizeFeeSection(bool fMinimize)
{
...
void ConvertCoinsDialog2::on_buttonChooseFee_clicked()
{
    minimizeFeeSection(false);
}

void ConvertCoinsDialog2::on_buttonMinimizeFee_clicked()
{
    updateFeeMinimizedLabel();
    minimizeFeeSection(true);
}

void ConvertCoinsDialog2::updateFeeSectionControls()
{
...
void ConvertCoinsDialog2::updateFeeMinimizedLabel()
{
    if(!model || !model->getOptionsModel())
        return;
...
void ConvertCoinsDialog2::updateCoinControlState()
{
...
void ConvertCoinsDialog2::updateNumberOfBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state) {
    if (sync_state == SynchronizationState::POST_INIT) {
        updateSmartFeeLabel();
    }
}

CAmountType ConvertCoinsDialog2::getFeeType()
{
    return BOND;
}

void ConvertCoinsDialog2::updateSmartFeeLabel()
{
    if(!model || !model->getOptionsModel())
        return;
    updateCoinControlState();
...
void ConvertCoinsDialog2::coinControlClipboardQuantity()
{
    GUIUtil::setClipboard(ui->labelCoinControlQuantity->text());
}

// Coin Control: copy label "Amount" to clipboard
void ConvertCoinsDialog2::coinControlClipboardAmount()
{
    GUIUtil::setClipboard(ui->labelCoinControlAmount->text().left(ui->labelCoinControlAmount->text().indexOf(" ")));
}

// Coin Control: copy label "Fee" to clipboard
void ConvertCoinsDialog2::coinControlClipboardFee()
{
    GUIUtil::setClipboard(ui->labelCoinControlFee->text().left(ui->labelCoinControlFee->text().indexOf(" ")).replace(ASYMP_UTF8, ""));
}

// Coin Control: copy label "After fee" to clipboard
void ConvertCoinsDialog2::coinControlClipboardAfterFee()
{
    GUIUtil::setClipboard(ui->labelCoinControlAfterFee->text().left(ui->labelCoinControlAfterFee->text().indexOf(" ")).replace(ASYMP_UTF8, ""));
}

// Coin Control: copy label "Bytes" to clipboard
void ConvertCoinsDialog2::coinControlClipboardBytes()
{
    GUIUtil::setClipboard(ui->labelCoinControlBytes->text().replace(ASYMP_UTF8, ""));
}

// Coin Control: copy label "Dust" to clipboard
void ConvertCoinsDialog2::coinControlClipboardLowOutput()
{
    GUIUtil::setClipboard(ui->labelCoinControlLowOutput->text());
}

// Coin Control: copy label "Change" to clipboard
void ConvertCoinsDialog2::coinControlClipboardChange()
{
    GUIUtil::setClipboard(ui->labelCoinControlChange->text().left(ui->labelCoinControlChange->text().indexOf(" ")).replace(ASYMP_UTF8, ""));
}

// Coin Control: settings menu - coin control enabled/disabled by user
void ConvertCoinsDialog2::coinControlFeatureChanged(bool checked)
{
...
void ConvertCoinsDialog2::coinControlButtonClicked()
{
    auto dlg = new CoinControlDialog(*m_coin_control, model, platformStyle);
    connect(dlg, &QDialog::finished, this, &ConvertCoinsDialog2::coinControlUpdateLabels);
    GUIUtil::ShowModalDialogAsynchronously(dlg);
}

// Coin Control: checkbox custom change address
void ConvertCoinsDialog2::coinControlChangeChecked(int state)
{
...
void ConvertCoinsDialog2::coinControlChangeEdited(const QString& text)
{
...
void ConvertCoinsDialog2::coinControlUpdateLabels()
{
    if (!model || !model->getOptionsModel())
        return;
