...
{
    // return if URI is not valid or is no peerfed: URI
    if(!uri.isValid() || uri.scheme() != QString("peerfed"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!BitcoinUnits::parse(BitcoinUnit::CASH, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("peerfed:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
bool isDust(interfaces::Node& node, const QString& address, const CAmount& amount, const CAmountType& amountType)
{
    CTxDestination dest = DecodeDestination(address.toStdString());
    CScript script = GetScriptForDestination(dest);
    CTxOut txOut(amountType, amount, script);
...
bool openBitcoinConf()
{
    ...
    /* Open peerfed.conf with the associated application */
