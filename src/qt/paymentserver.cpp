...
const QString BITCOIN_IPC_PREFIX("peerfed:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // peerfed: URI
        {
    ...
    optionsModel(nullptr)
{
    ...
    // on Mac: sent when you click peerfed: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start peerfed: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling peerfed: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("peerfed://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'peerfed://' is not a valid URI. Use 'peerfed:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // peerfed: URI
    {
