...
public:
    ...
    CAmounts getBestTotalSupply() override
    {
        const CBlockIndex* tip = WITH_LOCK(::cs_main, return chainman().ActiveChain().Tip());
    ...
    int64_t getBestInterestRate() override
    {
        const CBlockIndex* tip = WITH_LOCK(::cs_main, return chainman().ActiveChain().Tip());
        return tip ? tip->GetInterestRate() : 0;
    }
    ...
    std::unique_ptr<Handler> handleNotifyBlockTip(NotifyBlockTipFn fn) override
    {
        return MakeHandler(::uiInterface.NotifyBlockTip_connect([fn](SynchronizationState sync_state, const CBlockIndex* block) {
            fn(sync_state, BlockTip{block->nHeight, block->GetBlockTime(), block->GetBlockHash(), block->scaleFactor, block->GetInterestRate()},
...
public:
    ...
    std::optional<CAmountScaleFactor> findScaleFactorForHash(const uint256& block_hash) override
    {
        LOCK(::cs_main);
        const CBlockIndex* index = chainman().m_blockman.LookupBlockIndex(block_hash);
        return index ? std::optional{index->scaleFactor} : std::nullopt;
    }
    std::optional<CAmountScaleFactor> findScaleFactorAtHeight(const int& height) override
    {
        std::optional<int> current_height = getHeight();
        return (current_height && current_height >= height) ? findScaleFactorForHash(getBlockHash(height)) : std::nullopt;
    }
