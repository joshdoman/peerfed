 ...
 */
static const char* BITCOIN_PID_FILENAME = "peerfedd.pid";
...
void SetupServerArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-gen", strprintf("Generate coins (default: %u)", DEFAULT_GENERATE), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-genproclimit", strprintf("Set the number of threads for coin generation if enabled (-1 = all cores, default: %d)", DEFAULT_GENERATE_THREADS), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-mempoolconversionbuffer=<n>", strprintf("Use a <n> bips buffer when checking if conversion is valid for the mempool (default: %d)", DEFAULT_MEMPOOL_CONVERSION_BUFFER), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-mempoolexistingconversionschecklastnblocks=<n>", strprintf("Remove conversion transactions from memory pool if invalid for last <n> blocks (default: %d)", DEFAULT_MEMPOOL_EXISTING_CONVERSIONS_CHECK_LAST_N_BLOCKS), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-mempoolexpiry=<n>", strprintf("Do not keep transactions in the mempool longer than <n> hours (default: %u)", DEFAULT_MEMPOOL_EXPIRY_HOURS), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-mempoolminfeechecklastnblocks=<n>", strprintf("Add transactions to memory pool if normalized fee exceeds min fee using the best conversion rate across last <n> blocks (default: %d)", DEFAULT_MEMPOOL_MIN_FEE_CHECK_LAST_N_BLOCKS), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-mempoolnewconversionschecklastnblocks=<n>", strprintf("Add conversion transactions to memory pool if valid within last <n> blocks (default: %d)", DEFAULT_MEMPOOL_NEW_CONVERSIONS_CHECK_LAST_N_BLOCKS), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
bool AppInitParameterInteraction(const ArgsManager& args, bool use_syscall_sandbox)
{
    ...
    if (args.IsArgSet("-sandbox") && !args.IsArgNegated("-sandbox")) {
        ...
        LogPrintf("Experimental syscall sandbox enabled (-sandbox=%s): peerfedd will terminate if an unexpected (not allowlisted) syscall is invoked.\n", sandbox_arg);
    }
#endif // USE_SYSCALL_SANDBOX
