...
FUZZ_TARGET(script_bitcoin_consensus)
{
    ...
    const CAmountType moneyType = CASH;
    ...
    (void)bitcoinconsensus_verify_script_with_amount(random_bytes_1.data(), random_bytes_1.size(), moneyType, money, random_bytes_2.data(), random_bytes_2.size(), n_in, flags, err_p);
}
