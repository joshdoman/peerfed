...
The bpftrace examples contain a relative path to the `peerfedd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `peerfedd` binary. The paths in the examples can be changed, for
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/peerfedd
```
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/peerfedd
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/peerfedd
```
