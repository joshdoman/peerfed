...
""" Interactive peerfedd P2P network traffic monitor utilizing USDT and the
...
def main(peerfedd_path):
    peers = dict()

    peerfedd_with_usdts = USDT(path=str(peerfedd_path))

    # attaching the trace functions defined in the BPF program to the tracepoints
    peerfedd_with_usdts.enable_probe(
        probe="inbound_message", fn_name="trace_inbound_message")
    peerfedd_with_usdts.enable_probe(
        probe="outbound_message", fn_name="trace_outbound_message")
    bpf = BPF(text=program, usdt_contexts=[peerfedd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE:", sys.argv[0], "path/to/peerfedd")
        exit()
    path = sys.argv[1]
    main(path)
