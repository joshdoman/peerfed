This directory contains integration tests that test peerfedd and its
...
peerfedd and peerfed-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (bitcoin-util, peerfed-tx, ...).
...
The P2P and RPC ports used by the peerfedd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another peerfedd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its peerfedd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other peerfedd processes are running.

On linux, the test framework will warn if there is another
peerfedd process running when the tests are started.

If there are zombie peerfedd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
peerfedd processes running on the system, so should not be used if any non-test
peerfedd processes are being run.**
...
sure peerfedd processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and peerfedd `debug.log`s will all be dumped
...
`test_framework.log` and peerfedd `debug.log`s can be combined into a single
...
call methods that interact with the peerfedd nodes-under-test.

If further introspection of the peerfedd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/peerfedd.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/peerfedd <pid>
```
