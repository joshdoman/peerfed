...
# Objects that map to peerfedd objects, which can be serialized/deserialized
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in peerfedd
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class msg_version:
    ...
    def deserialize(self, f):
        ...
        # But, unconditionally check it to match behaviour in peerfedd
        try:
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in peerfedd indicates these should be deserialized as blocks
