...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave peerfedds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop peerfedds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use peerfed-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_peerfedd = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "peerfedd" + config["environment"]["EXEEXT"],
        )
        fname_bitcoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "peerfed-cli" + config["environment"]["EXEEXT"],
        )
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: peerfedds were not stopped and may still be running")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'peerfedd', self.options.peerfedd) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'peerfed-cli', self.options.bitcoincli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a peerfedd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple peerfedd test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as peerfedd's debug.log with microprecision (so log files can be concatenated and sorted)
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    peerfedd=self.options.peerfedd,
    ...
    def skip_if_no_peerfedd_tracepoints(self):
        """Skip the running test if peerfedd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("peerfedd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_peerfedd_zmq(self):
        """Skip the running test if peerfedd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("peerfedd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if peerfed-wallet has not been compiled."""
    ...
    def skip_if_no_cli(self):
        ...
        if not self.is_cli_compiled():
            raise SkipTest("peerfed-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether peerfed-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether peerfed-wallet was compiled."""
