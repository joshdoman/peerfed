...
- [Development guidelines](#development-guidelines)
    - [General PeerFed Core](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
PeerFed Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while peerfedd is running with the `logging`
RPC.  For instance, launching peerfedd with `-debug` or `-debug=1` will turn on
...
PeerFed Core is a multi-threaded application, and deadlocks or other
...
i.e. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run peerfedd.

You can then use the `-debug=lock` configuration option at peerfedd startup or
`peerfed-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `peerfed-cli logging [] '["lock"]'`.
...
$ valgrind -v --leak-check=full src/peerfedd -printtoconsole
...
To profile a running peerfedd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep peerfedd` -- sleep 60
```
...
PeerFed Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`peerfedd`)](https://doxygen.peerfed.org/peerfedd_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
  : Started from `main()` in `peerfedd.cpp`. Responsible for starting up and
  shutting down the application.

- [ThreadImport (`b-loadblk`)](https://doxygen.peerfed.org/namespacenode.html#ab4305679079866f0f420f7dbf278381d)
  : Loads blocks from `blk*.dat` files or `-loadblock=<file>` on startup.

- [CCheckQueue::Loop (`b-scriptch.x`)](https://doxygen.peerfed.org/class_c_check_queue.html#a6e7fa51d3a25e7cb65446d4b50e6a987)
  : Parallel script validation threads for transactions in blocks.

- [ThreadHTTP (`b-http`)](https://doxygen.peerfed.org/httpserver_8cpp.html#abb9f6ea8819672bd9a62d3695070709c)
  : Libevent thread to listen for RPC and REST connections.

- [HTTP worker threads(`b-httpworker.x`)](https://doxygen.peerfed.org/httpserver_8cpp.html#aa6a7bc27265043bc0193220c5ae3a55f)
  : Threads to service RPC and REST requests.

- [Indexer threads (`b-txindex`, etc)](https://doxygen.peerfed.org/class_base_index.html#a96a7407421fbf877509248bbe64f8d87)
  : One thread per indexer.

- [SchedulerThread (`b-scheduler`)](https://doxygen.peerfed.org/class_c_scheduler.html#a14d2800815da93577858ea078aed1fba)
...
- [TorControlThread (`b-torcontrol`)](https://doxygen.peerfed.org/torcontrol_8cpp.html#a52a3efff23634500bb42c6474f306091)
  : Libevent thread for tor connections.

- Net threads:

  - [ThreadMessageHandler (`b-msghand`)](https://doxygen.peerfed.org/class_c_connman.html#aacdbb7148575a31bb33bc345e2bf22a9)
  ...
  - [ThreadDNSAddressSeed (`b-dnsseed`)](https://doxygen.peerfed.org/class_c_connman.html#aa7c6970ed98a4a7bafbc071d24897d13)
    : Loads addresses of peers from the DNS.

  - ThreadMapPort (`b-mapport`)
    : Universal plug-and-play startup/shutdown.

  - [ThreadSocketHandler (`b-net`)](https://doxygen.peerfed.org/class_c_connman.html#a765597cbfe99c083d8fa3d61bb464e34)
    : Sends/Receives data from peers on port 8333.

  - [ThreadOpenAddedConnections (`b-addcon`)](https://doxygen.peerfed.org/class_c_connman.html#a0b787caf95e52a346a2b31a580d60a62)
    : Opens network connections to added nodes.

  - [ThreadOpenConnections (`b-opencon`)](https://doxygen.peerfed.org/class_c_connman.html#a55e9feafc3bab78e5c9d408c207faa45)
    : Initiates new connections to peers.

  - [ThreadI2PAcceptIncoming (`b-i2paccept`)](https://doxygen.peerfed.org/class_c_connman.html#a57787b4f9ac847d24065fbb0dd6e70f8)
...
However, in open source software such as PeerFed Core, where everyone uses
...
pay attention to for reviewers of PeerFed Core code.

General PeerFed Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: PeerFed Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of PeerFed Core, in which case
...
a PeerFed Core subtree, so that they can be integrated quickly. Cosmetic changes
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `peerfed-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `peerfed-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of PeerFed Core, the wallet was always
