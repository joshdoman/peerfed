...
For example:
    ...
    $ src/peerfed-cli -stdin walletpassphrase
    mysecretcode
    120
...
- #7550 `8b958ab` Input-from-stdin mode for peerfed-cli (laanwj)
...
- #7613 `3798e5d` Add autocomplete to peerfed-qt's console window (GamerSg)
...
- #8256 `af2421c` BUG: peerfed-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from peerfedd (sipa)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for peerfedd procs (MarcoFalke)
...
- #7744 `d8e862a` test_framework: detect failure of peerfedd startup (laanwj)
