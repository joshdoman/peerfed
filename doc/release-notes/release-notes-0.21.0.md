...
- A new `peerfed-cli -netinfo` command provides a network peer connections
...
- A new `peerfed-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `peerfed-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `peerfed-cli -getinfo` is now expanded to return a JSON
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `peerfed-wallet` tool which performs the salvage
...
wallet option), through the `peerfed-cli createwallet` or `peerfed-wallet
...
- #20198 Show name, format and if uses descriptors in peerfed-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add peerfed-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in peerfed-cli (laanwj)
...
- #18653 add coverage for peerfed-cli -rpcwait (jonatack)
...
- #18817 Document differences in peerfedd and peerfed-qt locale handling (practicalswift)
