...
Bitcoin Core 0.15.0 contains the following changes to the RPC interface and `peerfed-cli` for multi-wallet:

* When running Bitcoin Core with a single wallet, there are **no** changes to the RPC interface or `peerfed-cli`. All RPC calls and `peerfed-cli` commands continue to work as before.
* When running Bitcoin Core with multi-wallet, all *node-level* RPC methods continue to work as before. HTTP RPC requests should be send to the normal `<RPC IP address>:<RPC port>` endpoint, and `peerfed-cli` commands should be run as before. A *node-level* RPC method is any method which does not require access to the wallet.
* When running Bitcoin Core with multi-wallet, *wallet-level* RPC methods must specify the wallet for which they're intended in every request. HTTP RPC requests should be send to the `<RPC IP address>:<RPC port>/wallet/<wallet name>` endpoint, for example `127.0.0.1:8332/wallet/wallet1.dat`. `peerfed-cli` commands should be run with a `-rpcwallet` option, for example `peerfed-cli -rpcwallet=wallet1.dat getbalance`.
...
- `uptime` returns the total runtime of the `peerfedd` server since its last start (See [PR 10400](https://github.com/bitcoin/bitcoin/pull/10400)).
...
- Error codes have been updated to be more accurate for the following error cases (See [PR 9853](https://github.com/bitcoin/bitcoin/pull/9853)):
  ...
  - `removeprunedfunds` now returns RPC_WALLET_ERROR if `peerfedd` is unable to remove
  the transaction. Previously returned RPC_INTERNAL_ERROR.
  - `removeprunedfunds` now returns RPC_INVALID_PARAMETER if the transaction does not
  exist in the wallet. Previously returned RPC_INTERNAL_ERROR.
  - `fundrawtransaction` now returns RPC_INVALID_ADDRESS_OR_KEY if an invalid change
  address is provided. Previously returned RPC_INVALID_PARAMETER.
  - `fundrawtransaction` now returns RPC_WALLET_ERROR if `peerfedd` is unable to create
...
- #9672 `46311e7` Opt-into-RBF for RPC & peerfed-tx (luke-jr)
...
- #10400 `1680ee0` [RPC] Add an uptime command that displays the amount of time (in seconds) peerfedd has been running (rvelhote)
- #10683 `d81bec7` rpc: Move the `generate` RPC call to rpcwallet (laanwj)
- #10710 `30bc0f6` REST/RPC example update (Mirobit)
- #10747 `9edda0c` [rpc] fix verbose argument for getblock in peerfed-cli (jnewbery)
...
- #10017 `02d64bd` combine_logs.py - aggregates log files from multiple peerfedds during functional tests (jnewbery)
...
- #10636 `4bc853b` [qa] util: Check return code after closing peerfedd proc (MarcoFalke)
...
- #10090 `8e4f7e7` Update peerfed.conf with example for pruning (coinables)
...
- #10447 `f259263` Make peerfedd invalid argument error message specific (laanwj)
- #10495 `6a38b79` contrib: Update location of seeds.txt (laanwj)
- #10469 `b6b150b` Fixing typo in rpcdump.cpp help message (keystrike)
- #10451 `27b9931` contrib/init/peerfedd.openrcconf: Don't disable wallet by default (luke-jr)
...
- #10812 `c5904e8` [utils] Allow peerfed-cli's -rpcconnect option to be used with square brackets (jnewbery)
