...
- Passing an invalid `-rpcauth` argument now cause peerfedd to fail to start.  (#20461)
...
- A new `-rpcwaittimeout` argument to `peerfed-cli` sets the timeout
  in seconds to use with `-rpcwait`. If the timeout expires,
  `peerfed-cli` will report a failure. (#21056)
...
- bitcoin/bitcoin#21029 peerfed-cli: Correct docs (no "generatenewaddress" exists) (luke-jr)
...
### RPC and other APIs
- bitcoin/bitcoin#18335, bitcoin/bitcoin#21484 cli: Print useful error if peerfedd rpc work queue exceeded (LarryRuane)
...
- bitcoin/bitcoin#20939 fix `RELOC_SECTION` security check for peerfed-util (fanquake)
...
- bitcoin/bitcoin#22230 Fix MSVC linker /SubSystem option for peerfed-qt.exe (hebasto)
...
- bitcoin/bitcoin#22270 Add peerfed-util tests (+refactors) (MarcoFalke)
...
- bitcoin/bitcoin#21007 peerfedd: Add -daemonwait option to wait for initialization (laanwj)
...
- bitcoin/bitcoin#21695 contrib: Remove no longer used contrib/peerfed-qt.pro (hebasto)
...
- bitcoin/bitcoin#20913 Add manual page generation for peerfed-util (laanwj)
...
- bitcoin/bitcoin#21384 doc: add signet to peerfed.conf documentation (jonatack)
...
- bitcoin/bitcoin#21709 Update reduce-memory.md and peerfed.conf -maxconnections info (jonatack)
...
- bitcoin/bitcoin#21818 Fixup -coinstatsindex help, update peerfed.conf and files.md (jonatack)
...
- bitcoin/bitcoin#22080 add maxuploadtarget to peerfed.conf example (jarolrod)
...
- bitcoin/bitcoin#22393 Added info to peerfed.conf doc (bliotti)
