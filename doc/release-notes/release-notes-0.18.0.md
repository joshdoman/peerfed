...
`/Applications/Bitcoin-Qt` (on Mac) or `peerfedd`/`peerfed-qt` (on
Linux).
...
`contrib/init/peerfedd.service`, it has been changed to use
`/var/lib/peerfedd` as the data directory instead of
`~bitcoin/.bitcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/peerfedd` will
exist has enough space (check using `df -h /var/lib/peerfedd`), and
...
The systemd init file (`contrib/init/peerfedd.service`) has been changed
to use `/var/lib/peerfedd` as the data directory instead of
...
- A new [document](https://github.com/bitcoin/bitcoin/blob/master/doc/peerfed-conf.md)
  about the `peerfed.conf` file describes how to use it to configure
  Bitcoin Core.
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart peerfedd with the `-deprecatedrpc=generate`
...
- A new `peerfed-wallet` tool is now distributed alongside Bitcoin
...
- #13926 [Tools] peerfed-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like peerfedd (MarcoFalke)
...
- #14521 qt, docs: Fix `peerfed-qt -version` output formatting (hebasto)
...
- #15154 Configure: peerfed-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/peerfed-wallet (Sjors)
...
- #12246 Bugfix: Only run peerfed-tx tests when peerfed-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_peerfed` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14186 peerfed-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_peerfed.conf` message on startup if conf file exists (leishman)
...
- #14497 Add doc/peerfed-conf.md (hebasto)
...
- #15354 Add missing `peerfed-wallet` tool manpages (MarcoFalke)
