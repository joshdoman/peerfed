...
This allows running peerfedd without having to do any manual configuration.
...
`peerfed.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /bitcoinrpc>
        ...
        # optional bypass peerfedd rpc basic auth
...
Restarting peerfedd will no longer clear out the list of banned peers; instead
...
- #6600 `ef4945f` Include peerfed-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix peerfed-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of peerfed-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6387 `11576a5` [peerfed-cli] improve error output (Jonas Schnelli)
