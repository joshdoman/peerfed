...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `peerfedd -help` output for these two options for
...
- The `testnet` field in `peerfed-cli -getinfo` has been renamed to
...
- #15639 peerfed-wallet tool: Drop libbitcoin_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in peerfedd/qt (MarcoFalke)
...
- #16258 [MSVC]: Create the config.ini as part of peerfedd build (NicolasDorier)
...
- #16489 log: harmonize peerfedd logging (jonatack)
...
- #15513 add "sections" info to example peerfed.conf (fanquake)
...
- #16448 add note on precedence of options in peerfed.conf (fanquake)
...
- #16621 add default peerfed.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the peerfed-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, peerfed.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/peerfedd.cpp (ch4ot1c)
