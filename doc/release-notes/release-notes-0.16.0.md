...
- peerfed-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- peerfed-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.
...
- #11970 `b7450cd` Add test coverage for peerfed-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to peerfed-cli to allow RPC password to be read from standard input (jharvell)
...
- #10871 `c9a4aa8` Handle getinfo in peerfed-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop peerfedd (OmeGak)
...
- #10529 `331352f` Improve peerfedd systemd service file (Flowdalic)
