...
- A new `-asmap` configuration option has been added to diversify the
  ...
  `peerfedd help` for more information.  This option is experimental and
...
- The `peerfed-cli` utility used with the `-getinfo` parameter now
...
- The `peerfed-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17741 Included `test_peerfed-qt` in msvc build (sipsorcery)
...
- #18702 Fix ASLR for peerfed-cli on Windows (fanquake)
...
- #17119 Fix broken peerfed-cli examples (andrewtoth)
...
- #17503 Remove bitness from peerfed-qt help message and manpage (laanwj)
...
- #16115 On peerfedd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from peerfed-cli -getinfo (malevolent)
