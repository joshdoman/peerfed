...
Upgrading directly from a version of PeerFed Core that has reached its EOL is
possible, but it might take some time if the data directory needs to be migrated. Old
wallet versions of PeerFed Core are generally supported.

Compatibility
==============

PeerFed Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.15+, and Windows 7 and newer.  Bitcoin
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use PeerFed Core on
unsupported systems.

Notice of new option for transaction replacement policies
=========================================================

This version of PeerFed Core adds a new `mempoolfullrbf` configuration
...
fees, was realized in [PeerFed Core 0.12.0][] (February 2016) with the
...
default for years, and several contributors to PeerFed Core are
...
- To help prevent fingerprinting transactions created by the PeerFed Core wallet, change output
...
config files), so these settings will now apply to peerfedd, instead of being
