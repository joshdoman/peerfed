# CJDNS support in PeerFed Core

It is possible to run PeerFed Core over CJDNS, an encrypted IPv6 network that
...
done outside of PeerFed Core, similarly to a VPN (either in the host/OS or on
...
before it will work with your PeerFed Core node. This is described in steps
...
## Run PeerFed Core with CJDNS

Once you are connected to the CJDNS network, the following PeerFed Core
...
When enabled, this option tells PeerFed Core that it is running in an
...
IPv6 local network. This helps PeerFed Core perform better address management:
...
CJDNS support was added to PeerFed Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `peerfed-cli -addrinfo` to see the
...
## CJDNS-related information in PeerFed Core

There are several ways to see your CJDNS address in PeerFed Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `peerfed-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `peerfed-cli getpeerinfo`).
