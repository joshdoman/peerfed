...
`./`               | `peerfed.conf`        | User-defined [configuration settings](peerfed-conf.md) for `bitcoind` or `bitcoin-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
