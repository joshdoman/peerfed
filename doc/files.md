...
`./`               | `peerfed.conf`        | User-defined [configuration settings](peerfed-conf.md) for `peerfedd` or `peerfed-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `peerfedd.pid`        | Stores the process ID (PID) of `peerfedd` or `peerfed-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `peerfedd` or `peerfed-qt`; can be specified by `-debuglogfile` option
