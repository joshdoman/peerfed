Building PeerFed Core with Visual Studio
========================================

Introduction
---------------------
Visual Studio 2022 is minimum required to build PeerFed Core.

Solution and project files to build with `msbuild` or Visual Studio can be found in the `build_msvc` directory.

To build PeerFed Core from the command-line, it is sufficient to only install the Visual Studio Build Tools component.
...
Qt
---------------------
To build PeerFed Core with the GUI, a static build of Qt is required.
...
```cmd
msbuild build_msvc\peerfed.sln -property:Configuration=Release -maxCpuCount -verbosity:minimal
```

Alternatively, open the `build_msvc/peerfed.sln` file in Visual Studio.
